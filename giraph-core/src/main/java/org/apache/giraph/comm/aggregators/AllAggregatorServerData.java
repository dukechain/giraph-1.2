...
/**
 * Accepts aggregators and their values from previous superstep from master and
 * workers which own aggregators. Keeps data received from master so it could be
 * distributed later. Also counts the requests so we would know when we are done
 * receiving requests.
 * 
...
public class AllAggregatorServerData {
	...
	private static final Logger LOG = Logger
			.getLogger(AllAggregatorServerData.class);
	/** Map of broadcasted values from master */
	private final ConcurrentMap<String, Writable> broadcastedMap = Maps
			.newConcurrentMap();
	/** Map of registered reducers for current superstep */
	private final ConcurrentMap<String, ReduceOperation<Object, Writable>> reduceOpMap = Maps
			.newConcurrentMap();
	/**
	 * Counts the requests with final aggregators from master. It uses values
	 * from special aggregators (named AggregatorUtils.SPECIAL_COUNT_AGGREGATOR)
	 ...
	 */
	private final List<byte[]> masterData = Collections.synchronizedList(Lists
			.<byte[]> newArrayList());
	/**
	 * Counts the requests with final aggregators from other workers. It uses
	 * values from special aggregators (named
	 * AggregatorUtils.SPECIAL_COUNT_AGGREGATOR) to know how many requests it
	 * has to receive.
	 */
	 ...
	 *            Object value
	 */
	public void receiveValueFromMaster(String name, GlobalCommType type,
			Writable value) {
		switch (type) {
		case BROADCAST:
	 ...
	 * 
	 * @param data
	 *            Byte request with data received from master
	 */
	...
	public Iterable<byte[]> getDataFromMasterWhenReady(MasterInfo masterInfo) {
		LOG.info("wait for getDataFromMasterWhenReady, workIds "
	 ...
	 * @param broadcastedMapToFill
	 *            Broadcast map to fill out
	 * @param reducerMapToFill
	 *            Registered reducer map to fill out.
	 */
	public void fillNextSuperstepMapsWhenReady(Set<Integer> workerIds,
			Map<String, Writable> broadcastedMapToFill,
			Map<String, Reducer<Object, Writable>> reducerMapToFill) {
		LOG.info("wait for fillNextSuperstepMapsWhenReady, workIds "
				+ workerIds);
		...
		for (Entry<String, ReduceOperation<Object, Writable>> entry : reduceOpMap
				.entrySet()) {
			reducerMapToFill.put(entry.getKey(),
					new Reducer<>(entry.getValue()));
		}
