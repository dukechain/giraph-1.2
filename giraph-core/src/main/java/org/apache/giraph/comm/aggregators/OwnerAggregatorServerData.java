...
/**
 * Class for holding aggregators which current worker owns, and aggregating
 * partial aggregator values from workers.
 * 
 * Protocol: 1. Before the beginning of superstep, worker receives its
 * aggregators from master, and these aggregators will be registered to this
 * class. Multiple registrations can be called concurrently. 2. During the
 * superstep, whenever a worker finishes computation, it will send partial
 * aggregated values to worker owner. This class is used to help deserialize the
 * arriving aggregator values, and aggregate the values at the destination owner
 * worker; these can happen concurrently. (we know step 1. is finished before
 * anything from step 2. happens because other workers can't start computation
 * before they receive aggregators which this worker owns) 3. This class also
 * tracks the number of partial aggregator requests which worker received. In
 * the end of superstep, getMyAggregatorValuesWhenReady will be called to ensure
 * everything was received and get the values which need to be sent to master.
 * Because of this counting, in step 2. even if worker owns no aggregators, it
 * will still send a message without aggregator data. 4. In the end we reset to
 * prepare for the next superstep.
 */
public class OwnerAggregatorServerData {
	/** Class logger */
	private static final Logger LOG = Logger
			.getLogger(OwnerAggregatorServerData.class);
	/** Map of aggregators which current worker owns */
	private final ConcurrentMap<String, Reducer<Object, Writable>> myReducerMap = Maps
			.newConcurrentMap();
	/**
	 * Counts the requests with partial aggregated values from other workers. It
	 * uses GlobalCommType.SPECIAL_COUNT to know how many requests it has to
	 * receive.
	 */
	...
	/**
	 * Create initial value for a reducer. Used so requests would be able to
	 * deserialize data.
	 * 
	...
	/**
	 * Notify this object about the total number of requests which should arrive
	 * from one of the workers. Thread-safe.
	 * 
	 * @param requestCount
	 *            Number of requests which should arrive
	 * @param taskId
	 *            Task id of that worker
	 */
	 ...
	 */
	public Iterable<Map.Entry<String, Writable>> getMyReducedValuesWhenReady(
			Set<Integer> workerIds) {
		LOG.info("wait for getMyReducedValuesWhenReady, workIds " + workerIds);
		...
		return Iterables
				.transform(
						myReducerMap.entrySet(),
						new Function<Map.Entry<String, Reducer<Object, Writable>>, Map.Entry<String, Writable>>() {
							@Override
