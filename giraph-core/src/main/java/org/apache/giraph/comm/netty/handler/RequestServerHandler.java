...
public abstract class RequestServerHandler<R> extends
	...
	private static final Logger LOG = Logger
			.getLogger(RequestServerHandler.class);
	 ...
	 * Constructor
	 * 
	 * @param workerRequestReservedMap
	 *            Worker request reservation map
	 * @param conf
	 *            Configuration
	 * @param myTaskInfo
	 *            Current task info
	...
	public RequestServerHandler(
			...
			ImmutableClassesGiraphConfiguration conf, TaskInfo myTaskInfo,
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg)
		...
		if (workerRequestReservedMap.reserveRequest(request.getClientId(),
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("messageReceived: Processing client "
						+ request.getClientId() + ", " + "requestId "
						+ request.getRequestId() + ", " + request.getType()
						+ " took "
						+ Times.getNanosSince(TIME, startProcessingNanoseconds)
						+ " ns");
			}
			alreadyDone = AckSignalFlag.NEW_REQUEST;
		} else {
			LOG.info("messageReceived: Request id " + request.getRequestId()
					+ " from client " + request.getClientId()
					+ " was already processed, " + "not processing again.");
		}
		...
		int signal = flowControl.calculateResponse(alreadyDone,
				request.getClientId());
		buffer.writeInt(signal);
		ctx.write(buffer);
		// NettyServer is bootstrapped with auto-read set to true by default.
		// After
		// the first request is processed, we set auto-read to false. This
		// prevents
		// netty from reading requests continuously and putting them in off-heap
		// memory. Instead, we will call `read` on requests one by one, so that
		// the
	...
	public interface Factory {
		 ...
		 * should be called before any call to `#newHandle()`)
		 * 
		 * @param flowControl
		 *            reference to flow control used
		 */
