 ...
 * from the start. Instead we have a set of task ids, and each of those will, at
 * some point of time, give the information about how many events from it should
 * we expect. Barrier will be waiting for all the tasks to notify it about that
 * number of events, and than it will also wait for all the events to happen.
 * 
 * requirePermits() corresponds to task notifying us how many events from it to
 * expect, and releasePermits() notifies us about events happening.
 * 
 * This class is currently used during preparation of aggregators.
 * 
 * User must follow this protocol for concurrent access:
 * 
 * (1) an object instance is constructed (2) arbitrarily many times (2a)
 * concurrent calls to requirePermits(), releasePermits() and
 * waitForRequiredPermits() are issued (2b) waitForRequiredPermits() returns
 * 
 * Note that the next cycle of calls to requirePermits() or releasePermits()
 * cannot start until the previous call to waitForRequiredPermits() has
 * returned.
 * 
...
public class TaskIdsPermitsBarrier {
	...
	private static final Logger LOG = Logger
			.getLogger(TaskIdsPermitsBarrier.class);
	...
	/**
	 * Wait until permits have been required desired number of times, and all
	 * required permits are available
	 * 
	 * @param expectedTaskIds
	 *            List of task ids which we are waiting permits from
	 */
			...
			if (LOG.isInfoEnabled()) {
				if (arrivedTaskIds.size() < expectedTaskIds.size()) {
					...
					if (expectedTaskIds.size() - arrivedTaskIds.size() <= MAX_TASK_IDS_TO_LOG) {
						Sets.SetView<Integer> difference = Sets.difference(
								expectedTaskIds, arrivedTaskIds);
	...
	/**
	 * Require more permits. This will increase the number of times permits were
	 * required. Doesn't wait for permits to become available.
	 * 
	 * @param permits
	 *            Number of permits to require
	 * @param taskId
	 *            Task id which required permits
	 */
