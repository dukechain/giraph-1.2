...
public class ProgressableUtils {
	...
	/**
	 * When getting results with many threads, how many milliseconds to wait on
	 * each when looping through them
	 */
	 ...
	 * 
	 * @param executor
	 *            Executor which we are waiting for
	 * @param progressable
	...
	/**
	 * Wait for executorgroup to terminate, while periodically reporting
	 * progress
	 * 
	 * @param group
	 *            ExecutorGroup whose termination we are awaiting
	 * @param progressable
	 *            Progressable for reporting progress (Job context)
	 */
	public static void awaitTerminationFuture(EventExecutorGroup group,
			Progressable progressable) {
		waitForever(new FutureWaitable<>(group.terminationFuture()),
				progressable);
	}
	 ...
	 * 
	 * @param <T>
	 *            Type of the return value of the future
	 * @param future
	 *            Future
	 * @param progressable
	 *            Progressable for reporting progress (Job context)
	...
	/**
	 * Wait for {@link ChannelFuture} to finish, while periodically reporting
	 * progress.
	 * 
	 * @param future
	 *            ChannelFuture
			...
			if (waitable.isFinished()) {
				try {
					...
					throw new IllegalStateException(
							"waitForever: "
									+ "InterruptedException occurred while waiting for "
									+ waitable, e);
				}
			}
		}
	 ...
	 * @return Result of waitable
	 */
	private static <T> T waitFor(Waitable<T> waitable,
			Progressable progressable, int msecs, int msecsPeriod) {
	...
	/**
	 * Create {#link numThreads} callables from {#link callableFactory}, execute
	 * them and gather results.
	 * 
	 * @param callableFactory
	 *            Factory for Callables
	 * @param numThreads
		...
		while (!futures.isEmpty()) {
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					result = entry.getValue().get(MSEC_TO_WAIT_ON_EACH_FUTURE,
	 ...
	 */
	private abstract static class WaitableWithoutResult implements
			Waitable<Void> {
		@Override
	...
	/**
	 * {@link Waitable} for waiting on a {@link ChannelFuture} to terminate.
	 */
	private static class ChannelFutureWaitable extends WaitableWithoutResult {
		 ...
		 * 
		 * @param future
		 *            ChannelFuture which we want to wait for
		 */
	...
	private static class SemaphoreWaitable extends WaitableWithoutResult {
		...
		@Override
		public void waitFor(int msecs) throws InterruptedException {
			boolean acquired = semaphore.tryAcquire(permits, msecs,
					TimeUnit.MILLISECONDS);
