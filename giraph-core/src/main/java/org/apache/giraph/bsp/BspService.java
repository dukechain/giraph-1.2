...
public abstract class BspService<I extends WritableComparable, V extends Writable, E extends Writable>
	 ...
	 */
	public BspService(Mapper<?, ?, ?, ?>.Context context,
		...
		savedCheckpointBasePath = CheckpointingUtils.getCheckpointBasePath(
				getConfiguration(), restartJobId == null ? getJobId()
						: restartJobId);

		checkpointBasePath = CheckpointingUtils.getCheckpointBasePath(
				getConfiguration(), getJobId());
		...
		if (LOG.isInfoEnabled()) {
			LOG.info("BspService: Path to create to halt is "
					+ haltComputationPath);
		}
		if (LOG.isInfoEnabled()) {
		...
		if (LOG.isInfoEnabled()) {
			LOG.info("JobId = " + getJobId() + " restartJobId" + restartJobId);
		}

		// Trying to restart from the latest superstep
		// if (restartJobId != null && restartedSuperstep == UNSET_SUPERSTEP) {
		if (restartJobId != null && restartedSuperstep == UNSET_SUPERSTEP) {
			try {
		...
		if ((restartedSuperstep != UNSET_SUPERSTEP) && (restartedSuperstep < 0)) {
	...
	public final boolean isExsitPath(String path) {
		try {
			if (getZkExt().exists(path, false) != null)
				return true;
		} catch (KeeperException e) {
			throw new IllegalStateException(
					"Failed to verify whether path exsits "
	 ...
	 * 
	 * @param path
	 *            Path to parse for the superstep
	...
	public static long getSuperstepFromPath(String path) {
		...
		if (LOG.isTraceEnabled()) {
			LOG.trace("getSuperstepFromPath: Got path=" + path + ", start="
					+ foundSuperstepStart + ", end=" + endIndex);
		}
	...
	public final String getWorkerInfoUnhealthyPath(long attempt, long superstep) {
		return applicationAttemptsPath + "/" + attempt + SUPERSTEP_DIR + "/"
				+ superstep + WORKER_UNHEALTHY_DIR;
	}
	...
	public final String getWorkerWroteCheckpointPath(long attempt,
			long superstep) {
		return applicationAttemptsPath + "/" + attempt + SUPERSTEP_DIR + "/"
				+ superstep + WORKER_WROTE_CHECKPOINT_DIR;
	}
	...
	public final String getPartitionExchangePath(long attempt, long superstep) {
		return applicationAttemptsPath + "/" + attempt + SUPERSTEP_DIR + "/"
				+ superstep + PARTITION_EXCHANGE_DIR;
	}
	...
	public final String getSuperstepFinishedPath(long attempt, long superstep) {
		return applicationAttemptsPath + "/" + attempt + SUPERSTEP_DIR + "/"
				+ superstep + SUPERSTEP_FINISHED_NODE;
	}
	...
	/**
	 * In case when we restart another job this will give us a path to saved
	 * checkpoint.
	 * 
	 * @param superstep
	 *            superstep to use
	 ...
	 * 
	 * @param superstep
	 *            Set the manually restarted superstep
	 */
	...
	/**
	 * Get the master commanded job state as a JSONObject. Also sets the watches
	 * to see if the master commanded job state changes.
	 * 
	 * @return Last job state or null if none
	 */
	public final JSONObject getJobState() {
		try {
			getZkExt().createExt(masterJobStatePath, null, Ids.OPEN_ACL_UNSAFE,
		...
		try {
			List<String> childList = getZkExt().getChildrenExt(
					masterJobStatePath, true, true, true);
			if (childList.isEmpty()) {
				return null;
			}
			jobState = new String(getZkExt().getData(
	...
	public final long getApplicationAttempt() {
		...
		try {
			getZkExt().createExt(applicationAttemptsPath, null,
					Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT, true);
		} catch (KeeperException.NodeExistsException e) {
			LOG.info("getApplicationAttempt: Node " + applicationAttemptsPath
					+ " already exists!");
		} catch (KeeperException e) {
		...
		try {
			List<String> attemptList = getZkExt().getChildrenExt(
					applicationAttemptsPath, true, false, false);
			if (attemptList.isEmpty()) {
				cachedApplicationAttempt = 0;
			} else {
				cachedApplicationAttempt = Long.parseLong(Collections
						.max(attemptList));
			}
		} catch (KeeperException e) {
	...
	public final long getSuperstep() {
		...
		try {
			getZkExt().createExt(superstepPath, null, Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT, true);
		} catch (KeeperException.NodeExistsException e) {
			if (LOG.isInfoEnabled()) {
		...
		try {
			superstepList = getZkExt().getChildrenExt(superstepPath, true,
					false, false);
		} catch (KeeperException e) {
			throw new IllegalStateException("getSuperstep: KeeperException", e);
		} catch (InterruptedException e) {
	...
	/**
	 * 
	 * we know that there is superstep list on zk
	 */
	public final long getSuperstepBeforeMasterFailure() {
	    	String superstepPath = getSuperstepPath(getApplicationAttempt());
		...
		if (superstepList.isEmpty()) {
			return INPUT_SUPERSTEP;
		} else {
			return Long.parseLong(Collections.max(superstepList));
		}
	}
	...
	/**
	 * Set the cached superstep (should only be used for loading checkpoints or
	 * recovering from failure).
	 * 
	 * @param superstep
	 *            will be used as the next superstep iteration
	 */
	 ...
	 *            Will denote the new application attempt
	 */
	public final void setApplicationAttempt(long applicationAttempt) {
		...
		try {
			...
			throw new RuntimeException(
					"setApplicationAttempt: KeeperException - " + superstepPath,
					e);
		} catch (InterruptedException e) {
			throw new RuntimeException(
	...
	/**
	 * Register a BspEvent. Ensure that it will be signaled by catastrophic
	 * failure so that threads waiting on an event signal will be unblocked.
	 * 
	 * @param event
	...
	public final void process(WatchedEvent event) {
		...
		if ((event.getPath() == null) && (event.getType() == EventType.None)) {
			if (event.getState() == KeeperState.Disconnected) {
				// Watches may not be triggered for some time, so signal all
				// BspEvents
		...
		if (event.getPath().startsWith(masterJobStatePath)) {
			...
			eventProcessed = true;
		} else if ((event.getPath().contains(WORKER_HEALTHY_DIR) || event
				.getPath().contains(WORKER_UNHEALTHY_DIR))
	...
	/**
	 * For every worker this method returns unique number between 0 and N, where
	 * N is the total number of workers. This number stays the same throughout
	 * the computation. TaskID may be different from this number and task ID is
