...
public class BspServiceMaster<I extends WritableComparable, V extends Writable, E extends Writable>
		extends BspService<I, V, E> implements
		CentralizedServiceMaster<I, V, E>, ResetSuperstepMetricsObserver {
	 ...
	 */
	public BspServiceMaster(Mapper<?, ?, ?, ?>.Context context,
		...
		this.checkpointSupportedChecker = ReflectionUtils
				.newInstance(GiraphConstants.CHECKPOINT_SUPPORTED_CHECKER
						.get(conf));
	...
	@Override
	public void setJobState(ApplicationState state, long applicationAttempt,
	 ...
	 *            Attempt to start on
	 * @param desiredSuperstep
	 *            Superstep to restart from (if applicable)
	 * @param killJobOnFailure
	 *            if true, and the desired state is FAILED, then kill this job.
	 */
	private void setJobState(ApplicationState state, long applicationAttempt,
			long desiredSuperstep, boolean killJobOnFailure) {
	...
	/**
	 * Set the job state to FAILED. This will kill the job, and log exceptions
	 * to any observers.
	 * 
	 * @param reason
	 ...
	 * 
	 * @param inputFormat
	 *            The vertex/edge input format
	 * @param minSplitCountHint
	 *            Minimum number of splits to create (hint)
	 * @param inputSplitType
	 *            Type of input splits (for logging purposes)
	...
	private List<InputSplit> generateInputSplits(GiraphInputFormat inputFormat,
			int minSplitCountHint, InputType inputSplitType) {
		...
		float samplePercent = INPUT_SPLIT_SAMPLE_PERCENT
		...
		if (samplePercent != INPUT_SPLIT_SAMPLE_PERCENT.getDefaultValue()) {
			...
			if (LOG.isInfoEnabled()) {
				LOG.info(logPrefix + ": Got " + splits.size()
						+ " input splits for " + minSplitCountHint
						+ " input threads");
			}
			return splits;
		}
			...
			if (getConfiguration().isPureYarnJob()) {
				throw new RuntimeException(
						"BspServiceMaster (YARN profile) is "
								+ "FAILING this task, throwing exception to end job run.",
						e);
			} else {
				@SuppressWarnings("deprecation")
				org.apache.hadoop.mapred.JobClient jobClient = new org.apache.hadoop.mapred.JobClient(
						(org.apache.hadoop.mapred.JobConf) getContext()
								.getConfiguration());
				@SuppressWarnings("deprecation")
	...
	/**
	 * Parse the {@link WorkerInfo} objects from a ZooKeeper path (and
	 * children).
	 * 
	 * @param workerInfosPath
	...
	private List<WorkerInfo> getWorkerInfosFromPath(String workerInfosPath,
		...
		try {
			workerInfoPathList = getZkExt().getChildrenExt(workerInfosPath,
					watch, false, true);
		} catch (KeeperException e) {
			throw new IllegalStateException("getWorkers: Got KeeperException",
					e);
		} catch (InterruptedException e) {
			throw new IllegalStateException(
		...
		for (String workerInfoPath : workerInfoPathList) {
			...
			try {
				WritableUtils.readFieldsFromZnode(getZkExt(), workerInfoPath,
						true, null, workerInfo);
	...
	/**
	 * Get the healthy and unhealthy {@link WorkerInfo} objects for a superstep
	 * 
	 * @param superstep
	 *            superstep to check
	 * @param healthyWorkerInfoList
	 *            filled in with current data
	 * @param unhealthyWorkerInfoList
	 *            filled in with current data
	 */
	private void getAllWorkerInfos(long superstep,
			List<WorkerInfo> healthyWorkerInfoList,
			List<WorkerInfo> unhealthyWorkerInfoList) {
		String healthyWorkerInfoPath = getWorkerInfoHealthyPath(
				getApplicationAttempt(), superstep);
		String unhealthyWorkerInfoPath = getWorkerInfoUnhealthyPath(
				getApplicationAttempt(), superstep);

		try {
			getZkExt().createOnceExt(healthyWorkerInfoPath, null,
					Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT, true);
		} catch (KeeperException e) {
			throw new IllegalStateException("getWorkers: KeeperException", e);
		} catch (InterruptedException e) {
			throw new IllegalStateException("getWorkers: InterruptedException",
					e);
		}

		try {
			getZkExt().createOnceExt(unhealthyWorkerInfoPath, null,
					Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT, true);
		} catch (KeeperException e) {
			throw new IllegalStateException("getWorkers: KeeperException", e);
		} catch (InterruptedException e) {
		...
		List<WorkerInfo> currentHealthyWorkerInfoList = getWorkerInfosFromPath(
				healthyWorkerInfoPath, true);
		List<WorkerInfo> currentUnhealthyWorkerInfoList = getWorkerInfosFromPath(
				unhealthyWorkerInfoPath, false);
	...
	public List<WorkerInfo> checkWorkers() {
		...
		long failWorkerCheckMsecs = SystemTime.get().getMilliseconds()
		...
		while (SystemTime.get().getMilliseconds() < failWorkerCheckMsecs) {
			getContext().progress();
			getAllWorkerInfos(getSuperstep(), healthyWorkerInfoList,
					unhealthyWorkerInfoList);
			totalResponses = healthyWorkerInfoList.size()
					+ unhealthyWorkerInfoList.size();
			if ((totalResponses * 100.0f / maxWorkers) >= minPercentResponded) {
				failJob = false;
				break;
			}
			getContext().setStatus(
					getGraphTaskManager().getGraphFunctions() + " "
							...
							+ " needed to start superstep " + getSuperstep());
			...
			if (LOG.isInfoEnabled()) {
				LOG.info("checkWorkers: Only found "
						+ totalResponses
						+ " responses of "
						+ maxWorkers
						...
						+ eventWaitMsecs
						+ " msecs, "
						+ (failWorkerCheckMsecs - SystemTime.get()
								.getMilliseconds())
	 ...
	 *            The vertex/edge input format
	 * @param inputSplitType
	 *            Type of input split (for logging purposes)
	 * @return Number of splits. Returns -1 on failure to create valid input
	 *         splits.
	 */
	...
	public int createVertexInputSplits() {
		...
		VertexInputFormat<I, V, E> vertexInputFormat = getConfiguration()
	...
	public int createEdgeInputSplits() {
		...
		EdgeInputFormat<I, E> edgeInputFormat = getConfiguration()
	...
	private Collection<PartitionOwner> prepareCheckpointRestart(long superstep)
		...
		DataInputStream finalizedStream = fs.open(new Path(
				finalizedCheckpointPath));
		GlobalStats globalStats = new GlobalStats();
		globalStats.readFields(finalizedStream);
		updateCounters(globalStats);
		SuperstepClasses superstepClasses = SuperstepClasses
				.createToRead(getConfiguration());
		...
		for (int i = 0; i < prefixFileCount; ++i) {
			...
			for (long p = 0; p < partitions; ++p) {
				...
				PartitionOwner partitionOwner = new BasicPartitionOwner(
						partitionId, worker);
		...
		// Ordering appears to be important as of right now we rely on this
		// ordering
	...
	public boolean becomeMaster() {
		...
		String myBid = null;
		try {
			myBid = getZkExt().createExt(
					masterElectionPath + "/" + getHostnamePartitionId(), null,
					Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL, true);
		} catch (KeeperException e) {
		...
		while (true) {
			...
			try {
				if ((jobState != null)
						&& ApplicationState.valueOf(jobState
								.getString(JSONOBJ_STATE_KEY)) == ApplicationState.FINISHED) {
			...
			try {
				List<String> masterChildArr = getZkExt().getChildrenExt(
				...
				if (masterChildArr.get(0).equals(myBid)) {
					...
					globalCommHandler = new MasterGlobalCommHandler(
							new MasterAggregatorHandler(getConfiguration(),
									getContext()),
							new MasterInputSplitsHandler(getConfiguration()
					...
					masterServer = new NettyMasterServer(getConfiguration(),
							this, getContext(), getGraphTaskManager()
									.createUncaughtExceptionHandler());
					masterInfo.setInetSocketAddress(
							masterServer.getMyAddress(),
							masterServer.getLocalHostOrIp());
					masterInfo.setTaskId(getTaskPartition());
					masterClient = new NettyMasterClient(getContext(),
		...
		for (String finishedPath : workerFinishedPathList) {
			...
			try {
				...
				globalStats.addMessageBytesCount(workerFinishedInfoObj
						.getLong(JSONOBJ_NUM_MESSAGE_BYTES_KEY));
				if (conf.metricsEnabled()
						&& workerFinishedInfoObj.has(JSONOBJ_METRICS_KEY)) {
					WorkerSuperstepMetrics workerMetrics = new WorkerSuperstepMetrics();
					WritableUtils.readFieldsFromByteArray(Base64
							.decode(workerFinishedInfoObj
									.getString(JSONOBJ_METRICS_KEY)),
							workerMetrics);
					globalStats.addOocLoadBytesCount(workerMetrics
							.getBytesLoadedFromDisk());
					globalStats.addOocStoreBytesCount(workerMetrics
							.getBytesStoredOnDisk());
					// Find the lowest percentage of graph in memory across all
					// workers
					// for one superstep
					globalStats.setLowestGraphPercentageInMemory(Math.min(
							globalStats.getLowestGraphPercentageInMemory(),
							(int) Math.round(workerMetrics
	 ...
	 */
	private void printAggregatedMetricsToHDFS(long superstep,
		...
		try {
			...
			out = new PrintStream(fs.create(outFile), false, Charset
					.defaultCharset().name());
			aggregatedMetrics.print(superstep, out);
		} catch (IOException e) {
			throw new RuntimeException(
					"printAggregatedMetricsToHDFS: error creating metrics file",
					e);
		} finally {
			if (out != null) {
				out.close();
	 ...
	 * writing them to a finalized file. Also write out the master aggregated
	 * aggregator array from the previous superstep.
	 * 
	 * @param superstep
	 *            superstep to finalize
	 * @param chosenWorkerInfoList
	 *            list of chosen workers that will be finalized
	 * @throws IOException
	 * @throws InterruptedException
	 * @throws KeeperException
	 */
	private void finalizeCheckpoint(long superstep,
			List<WorkerInfo> chosenWorkerInfoList) throws IOException,
		...
		// <aggregator data>
		// <masterCompute data>
		FSDataOutputStream finalizedOutputStream = getFs().create(
				finalizedCheckpointPath);

		String superstepFinishedNode = getSuperstepFinishedPath(
				getApplicationAttempt(), superstep - 1);
		finalizedOutputStream.write(getZkExt().getData(superstepFinishedNode,
				false, null));
		...
		GiraphStats.getInstance().getLastCheckpointedSuperstep()
				.setValue(superstep);
	}

	/**
	 * Assign the partitions for this superstep. If there are changes, the
	 * workers will know how to do the exchange. If this was a restarted
	...
	private void assignPartitionOwners() {
		...
		if (getSuperstep() == INPUT_SUPERSTEP) {
			...
			try {
				...
				throw new IllegalStateException(
						"assignPartitionOwners: KeeperException on preparing",
						e);
			} catch (InterruptedException e) {
				throw new IllegalStateException(
			...
			partitionOwners = masterGraphPartitioner
					.generateChangedPartitionOwners(allPartitionStatsList,
							chosenWorkerInfoList, maxWorkers, getSuperstep());
		...
		if (!partitionOwners.isEmpty()) {
			String vertexExchangePath = getPartitionExchangePath(
					getApplicationAttempt(), getSuperstep());
			try {
				getZkExt().createOnceExt(vertexExchangePath, null,
						Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT, true);
			} catch (KeeperException e) {
				throw new IllegalStateException(
		...
		AddressesAndPartitionsWritable addressesAndPartitions = new AddressesAndPartitionsWritable(
				masterInfo, chosenWorkerInfoList, partitionOwners);
	 ...
	 * 
	 * @param partitionOwners
	 *            List of partition ids for current superstep
	 */
	 ...
	 * 
	 * @param chosenWorkerInfoHealthPath
	 *            Path to the healthy workers in ZooKeeper
	 * @param chosenWorkerInfoList
	 *            List of the healthy workers
	...
	private Collection<WorkerInfo> superstepChosenWorkerAlive(
			...
			List<WorkerInfo> chosenWorkerInfoList) throws KeeperException,
			InterruptedException {
		List<WorkerInfo> chosenWorkerInfoHealthyList = getWorkerInfosFromPath(
	...
	public void restartFromCheckpoint(long checkpoint) {
		...
		setJobState(ApplicationState.START_SUPERSTEP, getApplicationAttempt(),
				checkpoint);
	}

	/**
	 * Safely removes node from zookeeper. Ignores if node is already removed.
	 * Can only throw runtime exception if anything wrong.
	 * 
	 * @param path
	...
	public long getLastGoodCheckpoint() throws IOException {
		...
		if (lastCheckpointedSuperstep == -1) {
			try {
				...
				LOG.fatal(
						"getLastGoodCheckpoint: No last good checkpoints can be "
	 ...
	 * 
	 * @param finishedWorkerPath
	 *            Path to where the workers will register their hostname and id
	 * @param workerInfoList
	 *            List of the workers to wait for
	 * @param event
	 *            Event to wait on for a chance to be done.
	 * @param ignoreDeath
	 *            In case if worker died after making it through barrier, we
	 *            will ignore death if set to true.
	...
	private boolean barrierOnWorkerList(String finishedWorkerPath,
			List<WorkerInfo> workerInfoList, BspEvent event, boolean ignoreDeath) {
		try {
			getZkExt().createOnceExt(finishedWorkerPath, null,
					Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT, true);
		} catch (KeeperException e) {
			throw new IllegalStateException(
		...
		List<String> hostnameIdList = new ArrayList<String>(
				workerInfoList.size());
		for (WorkerInfo workerInfo : workerInfoList) {
			hostnameIdList.add(workerInfo.getHostnameId());
		}
		String workerInfoHealthyPath = getWorkerInfoHealthyPath(
				getApplicationAttempt(), getSuperstep());
		...
		int eventLoopTimeout = Math
				.min(taskTimeoutMsec, waitBetweenLogInfoMsec);
		boolean logInfoOnlyRun = false;
		List<WorkerInfo> deadWorkers = new ArrayList<>();
		while (true) {
			if (!logInfoOnlyRun) {
				try {
					finishedHostnameIdList = getZkExt().getChildrenExt(
							finishedWorkerPath, true, false, false);
				} catch (KeeperException e) {
					throw new IllegalStateException(
			...
			if (LOG.isInfoEnabled()
				...
				nextInfoMillis = System.currentTimeMillis()
						+ waitBetweenLogInfoMsec;
				LOG.info("barrierOnWorkerList: "
						+ finishedHostnameIdList.size() + " out of "
						+ workerInfoList.size()
						+ " workers finished on superstep " + getSuperstep()
						+ " on path " + finishedWorkerPath);
				if (workerInfoList.size() - finishedHostnameIdList.size() < MAX_PRINTABLE_REMAINING_WORKERS) {
					Set<String> remainingWorkers = Sets
							.newHashSet(hostnameIdList);
					remainingWorkers.removeAll(finishedHostnameIdList);
					LOG.info("barrierOnWorkerList: Waiting on "
							+ remainingWorkers);
				}
			}
			...
			if (!logInfoOnlyRun) {
				getContext().setStatus(
						getGraphTaskManager().getGraphFunctions() + " - "
								+ finishedHostnameIdList.size()
								+ " finished out of " + workerInfoList.size()
								+ " on superstep " + getSuperstep());
				...
				for (WorkerInfo deadWorker : deadWorkers) {
					if (!finishedHostnameIdList.contains(deadWorker
							.getHostnameId())) {
						LOG.error("barrierOnWorkerList: no results arived from "
								+ "worker that was pronounced dead: "
								+ deadWorker
								+ " on superstep "
								+ getSuperstep());
						return false;
			...
			try {
				deadWorkers.addAll(superstepChosenWorkerAlive(
						workerInfoHealthyPath, workerInfoList));
				if (!ignoreDeath && deadWorkers.size() > 0) {
					...
					if (!getConfiguration().useCheckpointing()) {
						...
						LOG.error("barrierOnWorkerList: Missing chosen "
								+ "workers " + deadWorkers + " on superstep "
								+ getSuperstep());
						// Log worker failure to command line
						getGraphTaskManager().getJobProgressTracker().logInfo(
								errorMessage);
					}
					return false;
				}
	 ...
	 * 
	 * @param removeableSuperstep
	 *            Supersteo to clean up
	 * @throws InterruptedException
	 */
	private void cleanUpOldSuperstep(long removeableSuperstep)
			throws InterruptedException {
		if (KEEP_ZOOKEEPER_DATA.isFalse(getConfiguration())
				&& (removeableSuperstep >= 0)) {
			String oldSuperstepPath = getSuperstepPath(getApplicationAttempt())
					+ "/" + removeableSuperstep;
			try {
				if (LOG.isInfoEnabled()) {
				...
				getZkExt().deleteExt(oldSuperstepPath, -1, true);
			} catch (KeeperException.NoNodeException e) {
	...
	private void coordinateInputSplits() {
		...
		if (!barrierOnWorkerList(inputSplitsWorkerDonePath,
				chosenWorkerInfoList, getInputSplitsWorkerDoneEvent(), false)) {
			throw new IllegalStateException(
					"coordinateInputSplits: Worker failed "
							+ "during input split (currently not supported)");
		}
		try {
			getZkExt().createExt(inputSplitsAllDonePath, null,
					Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT, false);
		} catch (KeeperException.NodeExistsException e) {
			LOG.info("coordinateInputSplits: Node " + inputSplitsAllDonePath
					+ " already exists.");
		} catch (KeeperException e) {
			throw new IllegalStateException(
	...
	/**
	 * Initialize aggregator at the master side before vertex/edge loading. This
	 * methods cooperates with other code to enables aggregation usage at
	 * INPUT_SUPERSTEP Other codes are: BSPServiceWorker:
	 * aggregatorHandler.prepareSuperstep in setup set aggregator usage in
	 * vertexReader and edgeReader
	 * 
	 * @throws InterruptedException
	 */
	private void initializeAggregatorInputSuperstep()
		...
		try {
			...
			LOG.fatal(
					"initializeAggregatorInputSuperstep: Failed in instantiation",
					e);
			throw new RuntimeException(
					"initializeAggregatorInputSuperstep: Failed in instantiation",
					e);
	...
	/**
	 * This is required before initialization and run of MasterCompute
	 * 
	 * @param superstep
	 *            superstep for which to run masterCompute
	 * @return Superstep classes set by masterCompute
	 */
	private SuperstepClasses prepareMasterCompute(long superstep) {
		GraphState graphState = new GraphState(superstep, GiraphStats
				.getInstance().getVertices().getValue(), GiraphStats
				.getInstance().getEdges().getValue(), getContext());
		SuperstepClasses superstepClasses = SuperstepClasses
				.createAndExtractTypes(getConfiguration());
	...
	@Override
	public SuperstepState coordinateSuperstep() throws KeeperException,
			InterruptedException {
		...
		if (chosenWorkerInfoList == null) {
			...
			Collections.sort(chosenWorkerInfoList,
					new Comparator<WorkerInfo>() {
						@Override
						public int compare(WorkerInfo wi1, WorkerInfo wi2) {
							return Integer.compare(wi1.getTaskId(),
									wi2.getTaskId());
						}
					});
			for (WorkerInfo workerInfo : chosenWorkerInfoList) {
		...
		GiraphStats.getInstance().getCurrentWorkers()
				.setValue(chosenWorkerInfoList.size());
		assignPartitionOwners();

		// Finalize the valid checkpoint file prefixes and possibly
		// the aggregators.
		if (checkpointStatus != CheckpointStatus.NONE) {
			String workerWroteCheckpointPath = getWorkerWroteCheckpointPath(
					getApplicationAttempt(), getSuperstep());
		...
		// We need to send aggregators to worker owners after new worker
		// assignments
		if (getSuperstep() >= 0) {
			globalCommHandler.getAggregatorHandler().sendDataToOwners(
					masterClient);
		}
		...
		String finishedWorkerPath = getWorkerFinishedPath(
				getApplicationAttempt(), getSuperstep());
		if (!barrierOnWorkerList(finishedWorkerPath, chosenWorkerInfoList,
				getSuperstepStateChangedEvent(), false)) {
			return SuperstepState.WORKER_FAILURE;
		}

		LOG.info("workers have been completed!");

		// Collect aggregator values, then run the master.compute() and
		// finally save the aggregator values
		globalCommHandler.getAggregatorHandler().prepareSuperstep();

		LOG.info("prepare before aggregatorTranslation!");

		aggregatorTranslation.prepareSuperstep();

		LOG.info("prepare before mastercompute!");

		SuperstepClasses superstepClasses = prepareMasterCompute(getSuperstep() + 1);
		doMasterCompute();

		LOG.info("Sucessfully collect aggregator values, then run the master.compute()!");
		...
		// the computation
		if (maxNumberOfSupersteps != GiraphConstants.MAX_NUMBER_OF_SUPERSTEPS
				.getDefaultValue()
				&& (getSuperstep() == maxNumberOfSupersteps - 1)) {
			if (LOG.isInfoEnabled()) {
				LOG.info("coordinateSuperstep: Finished "
		...
		LOG.info("Let everyone know the aggregated application state through the superstep finishing znode!");

		WritableUtils.writeToZnode(getZkExt(), superstepFinishedNode, -1,
				globalStats, superstepClasses);
		updateCounters(globalStats);

		LOG.info("begin to clean up old superstep!");
		...
		LOG.info("begin to getAggregatorHandler!");
	 ...
	 * the first user superstep. If restarting, the first checkpoint is after
	 * the frequency has been met.
	 * 
	 * @param superstep
	 *            Decide if checkpointing no this superstep
	...
	/**
	 * Returns false if job doesn't support checkpoints. Job may not support
	 * checkpointing if it does output during computation, uses static variables
	 * to keep data between supersteps, starts new threads etc.
	 * 
	 * @param conf
	 *            Immutable configuration of the job
	 * @param masterCompute
	 *            instance of master compute
	 * @return true if it is safe to checkpoint the job
	 */
	private boolean isCheckpointingSupported(GiraphConfiguration conf,
			MasterCompute masterCompute) {
		return checkpointSupportedChecker.isCheckpointSupported(conf,
				masterCompute);
	}

	/**
	 * This doMasterCompute is only called after masterCompute is initialized
	 */
	...
	private void cleanUpZooKeeper() {
		try {
			getZkExt().createExt(cleanedUpPath, null, Ids.OPEN_ACL_UNSAFE,
		...
		if ((getGraphTaskManager().getGraphFunctions() == GraphFunctions.ALL)
				|| (getGraphTaskManager().getGraphFunctions() == GraphFunctions.ALL_EXCEPT_ZOOKEEPER)) {
			maxTasks *= 2;
		}
		List<String> cleanedUpChildrenList = null;
		while (true) {
			try {
				cleanedUpChildrenList = getZkExt().getChildrenExt(
						cleanedUpPath, true, false, true);
				if (LOG.isInfoEnabled()) {
					LOG.info("cleanUpZooKeeper: Got "
							+ cleanedUpChildrenList.size() + " of " + maxTasks
							+ " desired children from " + cleanedUpPath);
				}
		...
		try {
			...
			LOG.error("cleanupZooKeeper: Failed to do cleanup of " + basePath
	...
	public void cleanup(SuperstepState superstepState) throws IOException {
		...
		String masterCleanedUpPath = cleanedUpPath + "/" + getTaskPartition()
				+ MASTER_SUFFIX;
		try {
			String finalFinishedPath = getZkExt().createExt(
					masterCleanedUpPath, null, Ids.OPEN_ACL_UNSAFE,
					CreateMode.PERSISTENT, true);
			if (LOG.isInfoEnabled()) {
		...
		if (isMaster) {
			...
			if (superstepState == SuperstepState.ALL_SUPERSTEPS_DONE
					&& GiraphConstants.CLEANUP_CHECKPOINTS_AFTER_SUCCESS
							.get(conf)) {
				boolean success = getFs().delete(new Path(checkpointBasePath),
						true);
				if (LOG.isInfoEnabled()) {
					LOG.info("cleanup: Removed HDFS checkpoint directory ("
							+ checkpointBasePath + ") with return = " + success
							+ " since the job " + getContext().getJobName()
							+ " succeeded ");
				}
			}
			if (superstepState == SuperstepState.CHECKPOINT_AND_HALT) {
				getFs().create(
						CheckpointingUtils.getCheckpointMarkPath(conf,
								getJobId()), true);
	...
	private void checkHealthyWorkerFailure(String failedWorkerPath) {
		...
		Collection<PartitionOwner> partitionOwners = masterGraphPartitioner
				.getCurrentPartitionOwners();
		String hostnameId = getHealthyHostnameIdFromPath(failedWorkerPath);
		for (PartitionOwner partitionOwner : partitionOwners) {
			WorkerInfo workerInfo = partitionOwner.getWorkerInfo();
			WorkerInfo previousWorkerInfo = partitionOwner
					.getPreviousWorkerInfo();
			if (workerInfo.getHostnameId().equals(hostnameId)
					|| ((previousWorkerInfo != null) && previousWorkerInfo
							.getHostnameId().equals(hostnameId))) {
				LOG.warn("checkHealthyWorkerFailure: "
						...
						+ "checkpointed superstep " + lastCheckpointedSuperstep);
	 ...
	 * 
	 * @param globalStats
	 *            Global statistics which holds new counter values
	 */
	private void updateCounters(GlobalStats globalStats) {
		...
		gs.getAggregateSentMessageBytes().increment(
				globalStats.getMessageBytesCount());
		gs.getAggregateOOCBytesLoaded().increment(
				globalStats.getOocLoadBytesCount());
		gs.getAggregateOOCBytesStored().increment(
				globalStats.getOocStoreBytesCount());
