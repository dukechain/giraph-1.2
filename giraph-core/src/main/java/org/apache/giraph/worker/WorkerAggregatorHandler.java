...
public class WorkerAggregatorHandler implements WorkerThreadGlobalCommUsage {
	...
	private static final Logger LOG = Logger
			.getLogger(WorkerAggregatorHandler.class);
	...
	private final Map<String, Reducer<Object, Writable>> reducerMap = Maps
			.newHashMap();
	 ...
	 *            Request processor for aggregators
	 */
	public void prepareSuperstep(
		...
		AllAggregatorServerData allGlobalCommData = serviceWorker
				.getServerData().getAllAggregatorData();
		// Wait for my aggregators
		Iterable<byte[]> dataToDistribute = allGlobalCommData
				.getDataFromMasterWhenReady(serviceWorker.getMasterInfo());

		LOG.info("after getDataFromMasterWhenReady");
		...
		try {
			...
			throw new IllegalStateException(
					"prepareSuperstep: "
							+ "IOException occurred while trying to distribute aggregators",
					e);
		}

		LOG.info("before fillNextSuperstepMapsWhenReady");

		// Wait for all other aggregators and store them
		allGlobalCommData.fillNextSuperstepMapsWhenReady(
				getOtherWorkerIdsSet(), broadcastedMap, reducerMap);
		if (LOG.isDebugEnabled()) {
	...
	public void finishSuperstep(
		...
		OwnerAggregatorServerData ownerGlobalCommData = serviceWorker
				.getServerData().getOwnerAggregatorData();
		// First send partial aggregated values to their owners and determine
		// which aggregators belong to this worker
		for (Map.Entry<String, Reducer<Object, Writable>> entry : reducerMap
				.entrySet()) {
			try {
				boolean sent = requestProcessor.sendReducedValue(
						entry.getKey(), entry.getValue().getCurrentValue());
				if (!sent) {
					// If it's my aggregator, add it directly
					ownerGlobalCommData.reduce(entry.getKey(), entry.getValue()
							.getCurrentValue());
				}
			} catch (IOException e) {
		...
		try {
			...
			throw new IllegalStateException(
					"finishSuperstep: "
							+ "IOException occurred while sending aggregators to owners",
					e);
		}

		// Wait to receive partial aggregated values from all other workers
		Iterable<Map.Entry<String, Writable>> myReducedValues = ownerGlobalCommData
				.getMyReducedValuesWhenReady(getOtherWorkerIdsSet());

		// Send final aggregated values to master
		GlobalCommValueOutputStream globalOutput = new GlobalCommValueOutputStream(
				false);
		for (Map.Entry<String, Writable> entry : myReducedValues) {
			try {
				int currentSize = globalOutput.addValue(entry.getKey(),
						GlobalCommType.REDUCED_VALUE, entry.getValue());
				if (currentSize > maxBytesPerAggregatorRequest) {
					requestProcessor.sendReducedValuesToMaster(globalOutput
							.flush());
				}
				progressable.progress();
			} catch (IOException e) {
		...
		try {
			...
			throw new IllegalStateException(
					"finishSuperstep: "
							+ "IOException occured while sending aggregators to master",
					e);
		}
	...
	public Set<Integer> getOtherWorkerIdsSet() {
		Set<Integer> otherWorkers = Sets
				.newHashSetWithExpectedSize(serviceWorker.getWorkerInfoList()
						.size());
		for (WorkerInfo workerInfo : serviceWorker.getWorkerInfoList()) {
			if (workerInfo.getTaskId() != serviceWorker.getWorkerInfo()
					.getTaskId()) {
	...
	/**
	 * Not thread-safe implementation of {@link WorkerThreadGlobalCommUsage}. We
	 * can use one instance of this object per thread to prevent synchronizing
	 * on each aggregate() call. In the end of superstep, values from each of
	 * these will be aggregated back to {@link WorkerThreadGlobalCommUsage}
	 */
	public class ThreadLocalWorkerGlobalCommUsage implements
		...
		public ThreadLocalWorkerGlobalCommUsage() {
			threadReducerMap = Maps
					.newHashMapWithExpectedSize(WorkerAggregatorHandler.this.reducerMap
							.size());

			UnsafeByteArrayOutputStream out = new UnsafeByteArrayOutputStream();
			UnsafeReusableByteArrayInput in = new UnsafeReusableByteArrayInput();

			for (Entry<String, Reducer<Object, Writable>> entry : reducerMap
					.entrySet()) {
				ReduceOperation<Object, Writable> globalReduceOp = entry
						.getValue().getReduceOp();

				ReduceOperation<Object, Writable> threadLocalCopy = WritableUtils
						.createCopy(out, in, globalReduceOp, conf);

				threadReducerMap.put(entry.getKey(), new Reducer<>(
						threadLocalCopy));
			}
		}
		...
		public void reduce(String name, Object value) {
			...
			if (reducer != null) {
				...
				throw new IllegalStateException("reduce: "
						+ AggregatorUtils.getUnregisteredAggregatorMessage(
								name, threadReducerMap.size() != 0, conf));
			}
		}
		...
		public void finishThreadComputation() {
			...
			for (Entry<String, Reducer<Object, Writable>> entry : threadReducerMap
					.entrySet()) {
				WorkerAggregatorHandler.this.reduceMerge(entry.getKey(), entry
